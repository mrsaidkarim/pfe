...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    // Méthode GET : afficher les données du médecin
    ...
    // Lecture de la ligne du médecin sélectionné depuis la table de la base de données
    $query = "SELECT * FROM Medecin WHERE IdMedecin=:id";
    ...
    $tele = $row["TeleMedecin"];
    $Specialite = $row["Specialite"];
    $email = $row["Email"];
} else {
    // Méthode POST : mettre à jour les données du médecin
    $id = $_POST["id"];
    $nom = $_POST["nom"];
    $prenom = $_POST["prenom"];
    $tele = $_POST["tele"];
    ...
    $email = $_POST["email"];
do{
    if (empty($id) || empty($nom) || empty($prenom) || empty($tele) || empty($Specialite) || empty($email)) {
        $errorMessage = "Tous les champs sont obligatoires";
        break;
    } else {
        $query = "UPDATE Medecin SET NomMedecin=:nom, PrenomMedecin=:prenom, TeleMedecin=:tele, Specialite=:specialite, Email=:email 
        WHERE IdMedecin=:id";
        $stmt = $pdo->prepare($query);
        ...
        if ($result) {
            $successMessage = "Médecin mis à jour avec succès";
            header("location: indexMedecin.php");
            exit;
        } else {
            $errorMessage = "Erreur lors de la mise à jour du médecin : " . $stmt->errorInfo()[2];
        }
    }
   }while(false);
}
